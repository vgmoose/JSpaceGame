...
public class Space extends JPanel implements MouseListener, KeyListener, MouseMotionListener
{
	...
	/**
	 * This class is a bit of a mess, but it basically does "everything else" in
	 * the game. The most interesting function is rotating the bitmap
	 * (makeRotationMatrix).
	 * 
	 * Other things it handles: - Joystick input (p1Move) - Bullet firing
	 * (p1Shoot) - Star drawing (renderStars) - Status bar drawing (renderTexts)
	 * - Decompressing sprites (decompress_bitmap) - Handling the menu at the
	 * title screen (doMenuAction)
	 * 
	 * It relies heavily on a SpaceGlobals struct defined in space.h. This is a
	 * carry over from the libwiiu pong example, but also I believe necesary
	 * since global variables don't seem to be able to be set(?)
	 **/
	...
	void p1Shoot(SpaceGlobals mySpaceGlobals)
	{
		...
		// (fabs(mySpaceGlobals.rstick.y) > sensitivity))
		{
			// if (fabs(mySpaceGlobals.rstick.x) > sensitivity)
			// xdif = mySpaceGlobals.p1X - (mySpaceGlobals.p1X +
			// (mySpaceGlobals.rstick.x * 18));
			// if (fabs(mySpaceGlobals.rstick.y) > sensitivity)
			// ydif = mySpaceGlobals.p1Y - (mySpaceGlobals.p1Y -
			// (mySpaceGlobals.rstick.y * 18));
			...
			// activateBullet(mySpaceGlobals, mySpaceGlobals.angle - 3.14159265,
			// mySpaceGlobals.p1X, mySpaceGlobals.p1Y);
			// shoot a bullet
			...
			for (xx = 0; xx < 20; xx++)
			{
				if (mySpaceGlobals.bullets[xx].active != 1)
				{
					...
					mySpaceGlobals.bullets[xx].m_x = 9 * TrigMath.my_sin(theta); // 9
																					// is
																					// the
																					// desired
																					// bullet
																					// speed
					mySpaceGlobals.bullets[xx].m_y = 9 * TrigMath.my_cos(theta); // we
																					// have
																					// to
																					// solve
																					// for
	...
	void p1Move(SpaceGlobals mySpaceGlobals)
	{
		...
		if (xdif < 0.1 && xdif > -0.1 && ydif < 0.1 && ydif > -0.1)
			return;
		...
		// update score if on a frame divisible by 60 (gain 10 points every
		// second)
		if (mySpaceGlobals.frame % 60 == 0)
		{
			increaseScore(mySpaceGlobals, 10);

			// if the score is at least 50 and a shot hasn't been fired yet,
			// display a message about shooting
	...
	void handleCollisions(SpaceGlobals mySpaceGlobals)
	{
		...
		// check enemies if they collide with the player or any of the 20 active
		// bullets
		int x;
		for (x = 0; x < 100; x++)
		{
			...
			{
				// collision checkin from here:
				// http://stackoverflow.com/a/1736741/1137828
				// check player
				...
				for (y = 0; y < 20; y++)
				{
					if (mySpaceGlobals.bullets[y].active == 1)
					{
						...
						if (sqMe1 * sqMe1 + sqMe2 * sqMe2 <= (7 + 1) * (7 + 1))
						{
							...
							increaseScore(mySpaceGlobals, 100); // 100 points
																// for killing
																// enemy
	...
	void makeScaleMatrix(int frame, int width, short[][] orig, short[][] targ, int transIndex)
	{
		...
		for (x = 0; x < width; x++)
		{
			...
			for (y = 0; y < width; y++)
			{
				// rotate the pixel by the angle into a new spot in the rotation
				// matrix
	...
	void handleExplosions(SpaceGlobals mySpaceGlobals)
	{
		...
		for (x = 0; x < 100; x++)
		{
			if (mySpaceGlobals.enemies[x].position.active > 1)
			{
				makeScaleMatrix((int) (mySpaceGlobals.enemies[x].position.active / 2.0), 23, mySpaceGlobals.enemy,
						mySpaceGlobals.enemies[x].rotated_sprite, 9);
		...
		if (mySpaceGlobals.playerExplodeFrame > 1)
		{
			makeScaleMatrix(mySpaceGlobals.playerExplodeFrame, 36, mySpaceGlobals.orig_ship,
					mySpaceGlobals.rotated_ship, mySpaceGlobals.transIndex);
	...
	void makeRotationMatrix(float angle, int width, short[][] orig, short[][] targ, int transIndex)
	{
		...
		for (ix = 0; ix < width; ix++)
		{
			...
			for (iy = 0; iy < width; iy++)
			{
				...
				// rotate the pixel by the angle into a new spot in the rotation
				// matrix
				int oldx = (int) ((ix - woffset) * TrigMath.my_cos(angle) + (iy - woffset) * TrigMath.my_sin(angle)
						+ woffset);
				int oldy = (int) ((ix - woffset) * TrigMath.my_sin(angle) - (iy - woffset) * TrigMath.my_cos(angle)
						+ woffset);
				...
				if (orig[oldx][oldy] == transIndex)
					continue;

				// TODO: crashes with this below line! When trying to assign to
				// target, but only after doing the above math
	...
	void renderEnemies(SpaceGlobals mySpaceGlobals)
	{
		...
		for (x = 0; x < 100; x++) // up to 100 enemies at once
		{
			if (mySpaceGlobals.enemies[x].position.active >= 1)
			{
				draw.drawBitmap(mySpaceGlobals.enemies[x].position.x, mySpaceGlobals.enemies[x].position.y, 23, 23,
						mySpaceGlobals.enemies[x].rotated_sprite, Images.enemy_palette);
			}
		}
	}
	...
	void decompress_sprite(int arraysize, int width, int height, short[] input, short[][] targ, int i)
	{
		...
		for (x = 0; x < arraysize; x += 2)
		{
			...
			////
			if (count <= 0) // if it's negative, -count is value, and value is
							// meaningless and advance by one
			{
				value = (short) (-1 * count);
				count = 1;
				x--; // subtract one, so next time it goes up by 2, putting us
						// at x+1
			}
	...
	void moveBullets(SpaceGlobals mySpaceGlobals)
	{
		...
		for (x = 0; x < 20; x++)
		{
			if (mySpaceGlobals.bullets[x].active == 1)
			{
				...
				if (mySpaceGlobals.bullets[x].x > xMaxBoundry || mySpaceGlobals.bullets[x].x < xMinBoundry
						|| mySpaceGlobals.bullets[x].y > yMaxBoundry || mySpaceGlobals.bullets[x].y < yMinBoundry + 20)
		...
		for (x = 0; x < 100; x++)
		{
			if (mySpaceGlobals.enemies[x].position.active == 1)
			{
				...
				makeRotationMatrix(mySpaceGlobals.enemies[x].angle, 23, mySpaceGlobals.enemy,
						mySpaceGlobals.enemies[x].rotated_sprite, 9);
				// makeScaleMatrix(3, 23, mySpaceGlobals.enemy,
				// mySpaceGlobals.enemies[x].rotated_sprite, 9);
	...
	void renderShip(SpaceGlobals mySpaceGlobals)
	{
		...
		if (mySpaceGlobals.playerExplodeFrame < 2)
			makeRotationMatrix(mySpaceGlobals.angle, 36, mySpaceGlobals.orig_ship, mySpaceGlobals.rotated_ship,
					mySpaceGlobals.transIndex);
	...
	{
		// cover up any old cursors (used to be needed before changing to draw
		// everything mode)
	...
	void doPasswordMenuAction(SpaceGlobals mySpaceGlobals)
	{
		...
		if (!(mySpaceGlobals.buttonA || mySpaceGlobals.buttonUp || mySpaceGlobals.buttonDown
				|| mySpaceGlobals.buttonLeft || mySpaceGlobals.buttonRight))
		{
			mySpaceGlobals.allowInput = 1;
		}
	...
	void addNewEnemies(SpaceGlobals mySpaceGlobals)
	{
		...
		for (xx = 0; xx < enemyCount; xx++)
		{
			if (mySpaceGlobals.enemies[xx].position.active == 0)
			{
				...
				mySpaceGlobals.enemies[xx].position.m_x = speed * TrigMath.my_sin(theta); // speed
																							// is
																							// the
																							// desired
																							// enemy
																							// speed
				mySpaceGlobals.enemies[xx].position.m_y = speed * TrigMath.my_cos(theta); // we
																							// have
																							// to
																							// solve
																							// for
	...
	void displayGameOver(SpaceGlobals mySpaceGlobals)
	{
		if (mySpaceGlobals.invalid == 1)
		{
			...
			if (mySpaceGlobals.dontKeepTrackOfScore != 1)
			{
				...
				String pass = "Lv " + mySpaceGlobals.level + 1 + "Password: "
						+ mySpaceGlobals.passwordList[mySpaceGlobals.level];
